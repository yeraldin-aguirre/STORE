...
// // Pruebas
// $fecha = "30-03-2022";
// $sDet = ejecutarSQL::consultar("SELECT d.*, v.Fecha FROM detalle AS d JOIN venta AS v
// ON d.NumPedido = v.NumPedido WHERE Fecha='" . substr($fecha, 0, 10) . "'");
...
// var_dump($fila1['PrecioProd'] * $fila1['CantidadProductos']);
// exit;
...
$pdf->Cell(170, 8, utf8_decode('Reporte de ' . $dVenta['tipo'] . 's #' . $id), 0, 1, 'C');
$pdf->Ln(20);
$pdf->SetFont("Arial", "b", 12);
$pdf->Cell(55, 5, utf8_decode('Fecha y hora del Reporte: '), 0);
...
$sDet = ejecutarSQL::consultar("SELECT d.*, v.Fecha FROM detalle AS d JOIN venta AS v
ON d.NumPedido = v.NumPedido WHERE Fecha='" . substr($dVenta['fecha'], 0, 10) . "'");
$cr = 0;
...
$pdf->Output('I', 'ReporteVenta-N' . $id . '.pdf');
