...
if ($documento != "" && $clave != "") {
    if ($radio == "option2") {
        $verAdmin = ejecutarSQL::consultar("SELECT * FROM usuarios WHERE usu_di='$documento' AND usu_pass='$clave'");
    ...
    if ($radio == "option1") {
        $verUser = ejecutarSQL::consultar("SELECT * FROM usuarios WHERE usu_di='$documento' AND usu_pass='$clave'");
