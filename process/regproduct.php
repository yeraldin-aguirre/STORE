...
$estruProd = consultasSQL::clean_string($_POST['prod-estru']);
		...
		if ($imgType == "image/jpeg" || $imgType == "image/png") {
			if (($imgSize / 1024) <= $imgMaxSize) {
				...
				if (move_uploaded_file($_FILES['img']['tmp_name'], "../assets/img-products/" . $imgFinalName)) {
					if (consultasSQL::InsertSQL("producto", "CodigoProd, NombreProd, CodigoCat, Precio, Descuento, Dimension, Estructura, Stock, NITProveedor, Imagen, Nombre, Estado", "'$codeProd','$nameProd','$cateProd','$priceProd', '$descProd', '$dimeProd','$estruProd','$stockProd','$codePProd','$imgFinalName','$adminProd', '$estadoProd'")) {
						echo '<script>
								swal({
